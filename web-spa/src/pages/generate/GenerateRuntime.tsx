...
const logger = (() => {
  const scopedWindow = typeof window !== 'undefined' ? (window as any) : undefined;
  const base = scopedWindow?.logger && typeof scopedWindow.logger === 'object'
    ? scopedWindow.logger
    : console;

  const pick = (method: 'debug' | 'info' | 'warn' | 'error') => {
    const candidate = (base as any)?.[method];
    if (typeof candidate === 'function') return candidate.bind(base);
    const fallback = (console as any)?.[method];
...
function exposeGenerateRuntimeGlobals() {
  const runtimeWindow = window as any;
  if (typeof loadDirectories === 'function') runtimeWindow.loadDirectories = loadDirectories;
  if (typeof selectDirectory === 'function') runtimeWindow.selectDirectory = selectDirectory;
  if (typeof toggleHazelcastGroupSelection === 'function') runtimeWindow.toggleHazelcastGroupSelection = toggleHazelcastGroupSelection;
  if (typeof toggleHazelcastGroupDetails === 'function') runtimeWindow.toggleHazelcastGroupDetails = toggleHazelcastGroupDetails;
  if (typeof selectAllHazelcastGroupTools === 'function') runtimeWindow.selectAllHazelcastGroupTools = selectAllHazelcastGroupTools;
  if (typeof deselectAllHazelcastGroupTools === 'function') runtimeWindow.deselectAllHazelcastGroupTools = deselectAllHazelcastGroupTools;
  if (typeof toggleRedisGroupSelection === 'function') runtimeWindow.toggleRedisGroupSelection = toggleRedisGroupSelection;
  if (typeof toggleRedisGroupDetails === 'function') runtimeWindow.toggleRedisGroupDetails = toggleRedisGroupDetails;
  if (typeof selectAllRedisGroupTools === 'function') runtimeWindow.selectAllRedisGroupTools = selectAllRedisGroupTools;
  if (typeof deselectAllRedisGroupTools === 'function') runtimeWindow.deselectAllRedisGroupTools = deselectAllRedisGroupTools;
  if (typeof toggleTableSelection === 'function') runtimeWindow.toggleTableSelection = toggleTableSelection;
  if (typeof toggleTableDetails === 'function') runtimeWindow.toggleTableDetails = toggleTableDetails;
  if (typeof selectAllTools === 'function') runtimeWindow.selectAllTools = selectAllTools;
  if (typeof deselectAllTools === 'function') runtimeWindow.deselectAllTools = deselectAllTools;
  if (typeof selectOnlyBasicTools === 'function') runtimeWindow.selectOnlyBasicTools = selectOnlyBasicTools;
  if (typeof closeSuccessModal === 'function') runtimeWindow.closeSuccessModal = closeSuccessModal;
  if (typeof goToManageServers === 'function') runtimeWindow.goToManageServers = goToManageServers;
}

let __generateRuntimeInitialized = false;

export function initGeneratePageRuntime(): void {
  const root = document.getElementById('generate-tab');
  if (!root) return;
  if (root.getAttribute('data-react-generate-init') === '1') return;
  root.setAttribute('data-react-generate-init', '1');

  try { exposeGenerateRuntimeGlobals(); } catch {}
  try { if (typeof initDirectoryPicker === 'function') initDirectoryPicker(); } catch {}
  try { if (typeof setupEventListeners === 'function') setupEventListeners(); } catch {}
  try { if (typeof setupTemplateFilters === 'function') setupTemplateFilters(); } catch {}
  try { if (typeof setupFileUpload === 'function') setupFileUpload(); } catch {}
  try { if (typeof applySaasDataSourceRestrictions === 'function') applySaasDataSourceRestrictions(); } catch {}
  try { if (typeof setupRouting === 'function') setupRouting(); } catch {}
  try { if (typeof handleInitialRoute === 'function') handleInitialRoute(); } catch {}
  try { if (typeof toggleDataSourceFields === 'function') toggleDataSourceFields(); } catch {}
  try { if (typeof updateWizardNavigation === 'function') updateWizardNavigation(); } catch {}

  __generateRuntimeInitialized = true;
}

export function isGeneratePageRuntimeInitialized(): boolean {
  return __generateRuntimeInitialized;
}
