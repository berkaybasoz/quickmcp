...
const SERVER_NAME = 'MYSQLDB-FINANCE';

test.describe('MySQL template', () => {
  ...
  test('generate MySQL server via UI', async ({ page }) => {
    await page.goto('/');

    await page.locator('input[name="dataSourceType"][value="mysql"]').check({ force: true });
    ...
    await page.fill('#dbPort', '3306');
    await page.fill('#dbName', 'quickmcp');
    await page.fill('#dbUser', 'root');
    await page.fill('#dbPassword', 'password');
    ...
    await page.fill('#serverDescription', 'MySQL quickmcp test');
