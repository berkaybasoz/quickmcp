...
class IntegratedMCPServer {
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
            ...
            if (name === 'quickmcp__get_server_info') {
                ...
                if (!serverInfo) {
                    throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Server '${serverId}' not found`);
                }
                return {
                    content: [{
                            type: 'text',
            ...
            if (name.includes('__')) {
                ...
                const tool = serverInfo.config.tools.find((t) => t.name === toolName);
                if (!tool) {
                    throw new types_js_1.McpError(types_js_1.ErrorCode.MethodNotFound, `Tool '${toolName}' not found in server '${serverId}'`);
                }
        ...
        this.server.setRequestHandler(types_js_1.ReadResourceRequestSchema, async (request) => {
            ...
            if (uri.startsWith('quickmcp://')) {
                ...
                const resource = serverInfo.config.resources.find((r) => r.uri === originalUri);
                if (!resource) {
                    throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Resource '${originalUri}' not found in server '${serverId}'`);
                }
            ...
            throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Unknown resource: ${uri}`);
        });
        this.server.setRequestHandler(types_js_1.GetPromptRequestSchema, async (request) => {
            ...
            if (name.includes('__')) {
                ...
                const prompt = serverInfo.config.prompts.find((p) => p.name === promptName);
                if (!prompt) {
                    throw new types_js_1.McpError(types_js_1.ErrorCode.MethodNotFound, `Prompt '${promptName}' not found in server '${serverId}'`);
                }
    ...
    async executeToolHandler(serverInfo, tool, args) {
    ...
    async generateResourceContent(serverInfo, resource) {
        const DATA = serverInfo.parsedData;
    ...
    processPromptTemplate(template, args) {
    ...
    searchTable(DATA, tableIndex, query, limit) {
        const data = DATA[tableIndex];
        const results = data.rows.filter((row) => row.some(cell => cell && cell.toString().toLowerCase().includes(query.toLowerCase()))).slice(0, limit);
        return {
    ...
    getAllFromTable(DATA, tableIndex, limit) {
        const data = DATA[tableIndex];
        return {
            headers: data.headers,
    ...
    filterTableByColumn(DATA, tableIndex, column, value, limit) {
        const data = DATA[tableIndex];
    ...
    async start(port = 3001) {
        ...
        this.app.use(express_1.default.raw({ type: 'application/json', limit: '10mb' }));
...
//# sourceMappingURL=integrated-mcp-server.js.map