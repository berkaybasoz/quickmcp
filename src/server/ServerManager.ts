...
export class ServerManager {
  ...
  constructor(baseDir: string = './generated-servers') {
  ...
  async buildServer(serverId: string): Promise<void> {
  ...
  async startServer(serverId: string, port?: number): Promise<void> {
    const instance = this.servers.get(serverId);
    if (!instance) {
      throw new Error(`Server ${serverId} not found`);
    }
    ...
    try {
      const childProcess = spawn('node', [distPath], {
        cwd: serverDir,
  ...
  async exportServer(serverId: string): Promise<string> {
    const instance = this.servers.get(serverId);
    if (!instance) {
      throw new Error(`Server ${serverId} not found`);
    }
  ...
  async updateServerConfig(serverId: string, newConfig: Partial<MCPServerConfig>): Promise<void> {
    const instance = this.servers.get(serverId);
    if (!instance) {
      throw new Error(`Server ${serverId} not found`);
    }
  ...
  private async saveServerMetadata(instance: ServerInstance): Promise<void> {
    const serverDir = path.dirname(instance.serverPath);
  ...
  private generateReadme(instance: ServerInstance): string {
