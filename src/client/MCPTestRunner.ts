...
export class MCPTestRunner {
  ...
  async runTestSuite(serverPath: string, testSuite: TestSuite): Promise<TestSuiteResult> {
  ...
  async generateTestSuite(serverPath: string, suiteName: string): Promise<TestSuite> {
    await this.client.connect(serverPath);
  ...
  private dataContains(actual: any, expected: any): boolean {
  ...
  private generateSamplePromptArgs(argumentsSchema: any[]): any {
    const args: any = {};
  ...
  private generateSampleValueFromDescription(description?: string): any {
    if (!description) return 'sample value';

    const lower = description.toLowerCase();
    if (lower.includes('query') || lower.includes('search')) return 'test query';
    if (lower.includes('name')) return 'sample name';
    if (lower.includes('focus')) return 'data patterns';
    if (lower.includes('number') || lower.includes('count')) return 10;
    if (lower.includes('boolean') || lower.includes('flag')) return true;
